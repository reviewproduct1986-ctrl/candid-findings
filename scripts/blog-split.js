...
/**
 * Split blogs.json into separate blog.<slug>.json files
 * Migrates from single file to multiple files structure
 */

const fs = require('fs');
const path = require('path');

function findBlogsFile() {
  const possiblePaths = [
...
function loadBlogsJson(filePath) {
  ...
  if (data.posts) {
    ...
    throw new Error('Unrecognized blogs.json structure');
  }
}
...
function main() {
  console.log('╔═══════════════════════════════════════════╗');
  console.log('║  Split blogs.json into separate files    ║');
  console.log('╚═══════════════════════════════════════════╝');
  console.log('');

  // Find blogs.json
  const blogsPath = findBlogsFile();
  
  if (!blogsPath) {
    console.error('❌ blogs.json not found!');
    console.error('');
    console.error('Searched in:');
    console.error('  - ./blogs.json');
    console.error('  - ./data/blogs.json');
    console.error('  - ./public/data/blogs.json');
    console.error('  - ../data/blogs.json');
    console.error('  - ../public/data/blogs.json');
    console.error('');
    process.exit(1);
  ...
  try {
    // Load blogs
    const blogs = loadBlogsJson(blogsPath);
    ...
    console.log('');
    console.log('💡 You can now delete blogs.json if everything looks good.');
    console.log('   (A backup was created: blogs.backup.json)');
    console.log('');
...
module.exports = { loadBlogsJson, saveBlogFiles, updateProductsWithSlugs };